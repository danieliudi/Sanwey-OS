...
//
// Caso de prospecção por voz: recebe o CAMINHO de um áudio já no Storage (ou
// um texto digitado), devolve a transcrição e um rascunho estruturado do que
// a IA entendeu sobre o que aconteceu numa visita/negociação. NÃO grava nada
// — quem grava é a tela, com o aceite do vendedor, pelo caminho normal do
// CRM (RLS do usuário). Clone estrutural de crm-ata-voz — mesmo padrão,
// domínio diferente (sales_cases, não leads.activities).
//
...
function systemPrompt(ctx: Record<string, unknown>): string {
  const linhas = [
    ctx.company ? `Cliente/prospect: ${ctx.company}` : null,
...
Você vai receber o relato do vendedor logo depois de uma visita ou negociação — fala espontânea, sem roteiro, com ruído, gíria e frases cortadas, contando o que aconteceu com um cliente ou prospect (ganhamos, perdemos, ou ainda em andamento). Sua tarefa é devolver EXCLUSIVAMENTE um objeto JSON neste formato:
...
// Mesma máscara de CPF do crm-ata-voz (que por sua vez duplica a do
// ai-assistant/_shared/ai-provider.ts, GAP 3 18/08/2026) — este arquivo
// também chama fetch direto pros providers, sem passar por callAIProvider.
// Só mascara o texto mandado pro modelo de ESTRUTURAÇÃO (segundo passo); o
// áudio em si já foi mandado inteiro pro Whisper transcrever. A transcrição
// devolvida pro vendedor conferir na tela continua completa, sem máscara —
// é dado do próprio sistema, protegido por RLS, não um prompt saindo pra fora.
...
Deno.serve(async (req: Request) => {
  ...
  // Mesma trilha de auditoria mínima do crm-ata-voz/ai-assistant/agent-runner
  // — só metadados, nunca conteúdo. Sem prompt/completion_tokens aqui
  // (Whisper não devolve usage) e sem `role` (esta function não recebe
  // roles, só o JWT do vendedor) — o resto do formato é o mesmo.
  const t0 = Date.now();
  try {
    const body = await req.json();
    const audioPath: string | undefined = body.audioPath;
    // Áudio ainda não gravado no Storage. É o caso do prospect que ainda não
    // é lead/cliente formal: ali o destino (se vira lead, se fica só
    // registrado) só é escolhido na tela de conferência, e lead-attachments
    // exige lead_id — ou seja, não há onde guardar o arquivo ANTES de
    // transcrever. Sem risco de vazamento: quem manda os bytes é o dono
    // deles, não um caminho apontando pro arquivo de outra pessoa (que é o
    // que audioPath protege via RLS do Storage).
