-- Menor da auditoria: os formulários públicos (LeadCaptureForm/
-- JobApplicationForm) só validam formato de e-mail no browser (type=email,
-- barra só lixo grosseiro) — nenhuma RPC valida no servidor, então uma
-- chamada direta (fora do navegador) grava e-mail malformado. Sem
-- segurança envolvida, mas o e-mail é usado como chave de dedupe/contato,
-- então vale validar formato antes de aceitar.
