    ...
    <div className="space-y-5">
      ...
      {currentUser?.role === "admin" && (
        <div className="print:hidden">
          <div className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: NEUTRAL.slate, letterSpacing: "0.08em" }}>
            Outras áreas
          </div>
          <DepartmentsOverview />
        </div>
  ...
  return (
    <div className="rounded-xl border p-5" style={{ background: "#FFFFFF", borderColor: "#E5E7EB", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
      ...
      <div className="flex items-start gap-8 flex-wrap">
        {stats.map(s => (
          <div key={s.label}>
            <div className="font-bold" style={{ fontSize: 22, color: NEUTRAL.graphite, lineHeight: 1 }}>{s.value}</div>
            <div className="text-xs mt-1" style={{ color: NEUTRAL.slate }}>{s.label}</div>
          </div>
        ))}
      </div>
    </div>
