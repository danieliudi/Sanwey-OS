...
const TABLE  = "marketing_deliverable_attachments";
const BUCKET = "deliverable-attachments";

export function useDeliverableAttachments(deliverableId) {
  ...
  const upload = useCallback(async (file, { uploadedBy } = {}) => {
    ...
    try {
      ...
      const path = `${deliverableId}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
